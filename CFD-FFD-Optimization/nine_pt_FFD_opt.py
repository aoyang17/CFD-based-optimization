...
parser.add_argument("--output", type=str, default="output_ninepoint")
...
def objCon(funcs, printOK):
    ...
    for i in range(nflowcase):
        ...
        if i == 4:
            funcs['cmz_con_'+ aeroProblems[i].name] = funcs[aeroProblems[i]["cmz"]] - cmcon
    if printOK:
...
optProb.addCon(f"cmz_con_{aeroProblems[4].name}", lower=0.0, upper=10.0, scale=10.0)
